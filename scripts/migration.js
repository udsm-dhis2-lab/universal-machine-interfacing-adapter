...
const patientResults = (results) => {
  ...
  Object.keys(mapping)?.forEach(key => {
    const data = mapping[key].map(d => results.find(({ assayNumber }) => assayNumber === d))
    if (data.length > 0) {
      result[key] = data
    } else {
...
const start = async () => {
  try {
    ...
    for (const dataArray of dataArrays) {
      const testedResults = dataArray.resultData.map(response => {
        return { ...response, result: response?.result?.split('^')?.join('')?.split('5')?.join(''), resultType: '' }
      })
